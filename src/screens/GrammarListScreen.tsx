        ...
        {grammarExpanded.map((lesson, index) => {
          ...
          return (
            <div 
              ...
              style={{ cursor: 'pointer', opacity: (isCompleted || isPremiumLocked) ? 0.8 : 1 }}
            >
              ...
              <p style={{ color: colors.textSecondary, fontSize: 14, margin: `0 0 ${spacing.md}px`, lineHeight: '1.4', flex: 1 }}>
                10 interactive exercises to master {lesson.title.toLowerCase()}.
              </p>
              <PrimaryButton 
                label={isPremiumLocked ? "Unlock Premium" : "Start Lesson"} 
                onPress={() => isPremiumLocked ? navigate('/premium') : navigate(`/grammar/${lesson.id}`)} 
