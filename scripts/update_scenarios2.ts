...
const allBlueprints = [
  ...
  },
  {
    ...
    sentences: [
      ['Qual è il prezzo per tre camicie?', 'What is the washing price for three shirts?', {grammarPoint: 'Irregular plural "camicie".'}],
...
// Using a slightly more robust regex to extract existing blueprint bodies
const objectRegex = /\{id: (\d+), category: '([^']*)', title: '((?:[^']|\\')*)', description: '((?:[^']|\\')*)', (?:difficulty: (\d+), )?focus: (\[[^\]]*\])\}/g;

const updatedBlueprintsStrings = [];
let match;
while ((match = objectRegex.exec(blueprintsRaw)) !== null) {
  const id = parseInt(match[1]);
  const category = match[2];
  const title = match[3];
  const description = match[4];
  const difficulty = match[5];
  const focus = match[6];
  
  const update = allBlueprints.find(b => b.id === id);
  if (!update) {
    console.warn('No update for scenario ID', id);
    updatedBlueprintsStrings.push(`  {id: ${id}, category: '${category}', title: '${title}', description: '${description}', ${difficulty ? `difficulty: ${difficulty}, ` : ''}focus: ${focus}}`);
    continue;
  }
  
  updatedBlueprintsStrings.push(`  {id: ${id}, category: '${category}', title: '${title}', description: '${description}', ${difficulty ? `difficulty: ${difficulty}, ` : ''}focus: ${focus}, phrases: ${JSON.stringify(update.phrases)}, sentences: ${JSON.stringify(update.sentences)}}`);
}
